...
Within IceCube software, the **tableio** module might be adapt-able to produce similar output, and if i3cols can include metadata alongside the written arrays, that project has the ability to emit units and other useful metadata. For now, to prove the concept, the extraction part of i3cols is very striaghtforward (just not "automatic") as written, but utilizing tableio should be strongly considered in the future.
